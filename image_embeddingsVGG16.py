...
#load the vgg-16 base model without the fully connected layers as we don't classify the images here
...
#count the iterations only for reference
count=0
...
for index, row in df.iterrows():
    ...
    if os.path.isfile(file):
        ...
        count=count+1
        #reference to check the processing progress
        print(count)
...
np.save("TempData/ImageEmbeddingVGG16.npy",np.squeeze(embedding))
...
df.to_excel("TempData/outputVGG16.xlsx", index = False)
